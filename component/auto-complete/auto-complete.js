...
export default class AutoComplete extends Component {
	...
	parseToJSX (name, searchText) {
		let lowerName = name.toLowerCase()
		let lowersearchText = searchText.toLowerCase()
		let list = lowerName.split(lowersearchText)

		let findedStrs = []
		let regexp = new RegExp(searchText, 'gi')
		findedStrs = name.match(regexp)

		let htmls = []
		list.forEach((item, index) => {
			htmls.push(item)
			if (index < list.length - 1) {
				htmls.push(<span className='highlight'>{findedStrs[index]}</span>)
			}
		})
	...
	parseToHTMLSuggestions (list, searchText) {
		...
		list.forEach((item) => {
			let suggestion = {
				id: item.id,
				html: this.parseToJSX(item.name, text)
			}
		...
		return (
			<div className='auto-complete-component' >
				<input className='search-input'
					...
					onBlur={this.clearInputValue.bind(this)} />
				{
