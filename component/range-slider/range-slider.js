...
export default class RangeSlider extends Component {
	...
	handleMouseMove (e) {
		if (this.state.isFromPointerDragging || this.state.isToPointerDragging) {
			...
			let boundaryLeft = 0
			let targetLeft = 0
			let ulWidth = this.ulItem.getBoundingClientRect().width
			let itemTotal = this.props.fieldList.length
			let boundaryRight = ulWidth / itemTotal * (itemTotal - 1)
			if (currentLeft <= boundaryLeft) { // 左侧边界
				targetLeft = boundaryLeft
			} else if (currentLeft >= boundaryRight) {
				targetLeft = boundaryRight
			} else {
	...
	render () {
		...
		return <div className='range-slider-component'>
			<ul className='clearfix' ref={(c) => { this.ulItem = c }} >
				{
